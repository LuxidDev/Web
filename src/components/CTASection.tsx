...
export default function CTASection() {
  ...
  return (
    <section id="get-started" className={`py-32 bg-gradient-to-b relative overflow-hidden ${
      darkMode
      ...
      <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
          ...
          <a href="#" className={`flex items-center gap-2 px-8 py-4 border rounded-xl transition-all ${
            darkMode
        ...
        <div className={`inline-flex items-center gap-3 px-6 py-4 border rounded-xl font-mono text-sm backdrop-blur-sm group hover:border-zinc-400 transition-colors ${
          darkMode
          ...
          <button onClick={handleCopy} className={`p-1.5 rounded transition-colors ${
            ...
            {copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className={`w-4 h-4 ${
              darkMode ? 'text-zinc-500 group-hover:text-zinc-300' : 'text-zinc-600 group-hover:text-zinc-800'
            }`} />}
          </button>
        </div>
      </div>
