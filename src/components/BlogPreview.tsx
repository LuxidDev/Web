...
const posts = [
  ...
  { title: 'Understanding AVE Architecture', excerpt: 'A deep dive into the Actions, Views and Entities pattern and why it makes your code better.', date: 'Nov 25, 2025', tag: 'Tutorial' },
    ...
    <section id="blog" className={`py-32 ${darkMode ? 'bg-black' : 'bg-white'}`}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {posts.map((post, i) => (
            <article key={i} className={`group p-6 border rounded-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer ${darkMode
                ? 'bg-zinc-900/50 border-zinc-800 hover:border-zinc-600'
                : 'bg-zinc-50 border-zinc-200 hover:border-zinc-400'
              }`}>
              <div className="flex items-center gap-3 mb-4">
                <span className={`px-2 py-1 text-xs rounded ${darkMode ? 'bg-zinc-800 text-zinc-300' : 'bg-zinc-200 text-zinc-700'
                  }`}>{post.tag}</span>
                <span className={`flex items-center gap-1 text-sm ${darkMode ? 'text-zinc-500' : 'text-zinc-600'
                  }`}>
                  <Calendar className="w-3 h-3" /> {post.date}
                </span>
              </div>
              <h3 className={`text-xl font-semibold mb-3 group-hover:opacity-80 transition-colors ${darkMode ? 'text-white' : 'text-black'
                }`}>{post.title}</h3>
              <p className={`text-sm leading-relaxed mb-4 ${darkMode ? 'text-zinc-400' : 'text-zinc-600'
                }`}>{post.excerpt}</p>
              <span className={`inline-flex items-center gap-1 text-sm transition-colors ${darkMode
        ...
        <a href="#" className={`md:hidden flex items-center justify-center gap-2 mt-8 transition-colors ${darkMode ? 'text-white hover:text-zinc-300' : 'text-black hover:text-zinc-700'
          }`}>
          View all posts <ArrowRight className="w-4 h-4" />
        </a>
      </div>
