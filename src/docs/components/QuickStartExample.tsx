...
export default function QuickStartExample() {
  ...
  return (
    <div className={`my-8 rounded-2xl overflow-hidden ${
      darkMode ? 'bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/20' : 'bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200'
    }`}>
      <div className="p-6">
...
$router->delete('/todos/{id}', [TodoAction::class, 'destroy']);`}
          ...
          <div className={`p-4 rounded-lg ${
            darkMode ? 'bg-zinc-800/50 border border-zinc-700' : 'bg-zinc-100 border border-zinc-300'
          }`}>
            <div className="flex items-center gap-2 mb-2">
              <Terminal className={`w-5 h-5 ${darkMode ? 'text-yellow-400' : 'text-yellow-600'}`} />
            ...
            <CodeExample
              ...
              language="bash"
              explanation="This single command generates Entity, Action, Migration, and Routes for a complete CRUD API."
            />
          </div>
        </div>
      </div>
